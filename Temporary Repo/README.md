...
Having access to a large variety of processors and file formats is essential. At the moment, IDA Pro offers the most vast range([2](https://hex-rays.com/ida-pro),[4](https://hackmag.com/security/nsa-ghidra/)), including support for certain exotic architectures like RISC-V and SPARC([7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)). Taking that into consideration, Ghidra comes close behind with support for most file formats([1](https://github.com/NationalSecurityAgency/ghidra/wiki/Frequently-asked-questions),[5](https://www.apriorit.com/dev-blog/366-software-reverse-engineering-tools),[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)), including modern ones, and for most processors including their variants, while allowing for the addition of new architectures or extending pre existing ones([1](https://github.com/NationalSecurityAgency/ghidra/wiki/Frequently-asked-questions),[4](https://hackmag.com/security/nsa-ghidra/)).<br>
Processors such as([1](https://github.com/NationalSecurityAgency/ghidra/wiki/Frequently-asked-questions)):
* Z80
* X86 16/32/64
* 6502
* ARM/AARCH64
...
In terms of decompilers, IDA Pro’s "Hex-Rays" is best thanks to its accuracy being comparable to that of a human([3](https://letsdefend.io/blog/top-[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)-reverse-engineering-tools?utm_source=perplexity),[5](https://www.apriorit.com/dev-blog/366-software-reverse-engineering-tools)). The technological advancements implemented through IDA 9.0 also introduced new decompilers for 32- and 64-bit RISC-V code([9](https://cybersecuritynews.com/ida-9-0-released/)), designed to modernise processes and increase capacities. Ghidra’s decompiler is powerful, but not as refined as Hex-Rays([3](https://letsdefend.io/blog/top-[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)-reverse-engineering-tools?utm_source=perplexity)).<br>
Furthermore, IDA Pro provides superior debugging and emulation for running and tracing target systems, particularly for Android and Windows([7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)).<br>
Currently IDA Pro delivers greater support for patching binaries, interactively changing characteristics of displayed data, renaming variables, functions, structures or changing data presentation and creating custom types([5](https://www.apriorit.com/dev-blog/366-software-reverse-engineering-tools),[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)).

Regarding graph generation, Ghidra offers a more informative, highly customizable and detailed visualisation of code, even marking “if … else” statements on the graph and making it more organised([4](https://hackmag.com/security/nsa-ghidra/)).<br>
For working teams, facilitating collaborative reverse engineering is important. Ghidra’s design supports working on the same project([3](https://letsdefend.io/blog/top-[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)-reverse-engineering-tools?utm_source=perplexity),[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193),[10](https://www.linkedin.com/advice/3/what-advantages-disadvantages-using-ghidra)), while IDA Pro requires certain plugins and the database files are not designed to be shared([7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)).

The support for cross-platform, while versatile, is implemented by few tools, like Ghidra, Rafare2 and JaDx, so that they work on macOS, Windows and Linux([3](https://letsdefend.io/blog/top-[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)-reverse-engineering-tools?utm_source=perplexity)).

Concerning binaries, Ghidra is faster and loads multiple binaries into projects simultaneously, resulting in an easier approach to following code between applications and libraries, by using multi-threading and multi-core properties of the CPU([7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)). For bigger files, 150 MB+, IDA Pro has a better optimization of the memory and CPU usage, it can lessen the processing time, being altogether faster than the other([4](https://hackmag.com/security/nsa-ghidra/),[7](https://medium.com/@Progsky/ghidra-vs-ida-pro-a-comparison-of-two-popular-reverse-engineering-tools-55223fad9193)).

The most significant cutting-age technology implemented recently is helping with pattern recognition accuracy and processing very large files due to integration of AI and machine learning in disassemblers. Models are being trained to increase code recognition patterns and data structures([6](https://www.retroreversing.com/disassemblers)).
A disassembler based on deep learning, DeepDi, proved to be simultaneously fast, highly accurate and efficient against obfuscated binaries, managing to discern valid instructions from the raw binary([13](https://www.cs.ucr.edu/~heng/pubs/DeepDi.pdf),[14](https://blog.deepbitstech.com/2020/06/deepdisassembly-blazing-fast-and.html)). For capturing and propagating instruction relations, it uses a neural network graphing model([13](https://www.cs.ucr.edu/~heng/pubs/DeepDi.pdf)). It has a very low false positive rate and false negative rate when identifying instructions, 0.02%/0.02%([11](https://www.deepbits.com/deepdi),[13](https://www.cs.ucr.edu/~heng/pubs/DeepDi.pdf)), and compared to IDA Pro, the CPU version is twice as fast and the GPU version is 350 times faster([11](https://www.deepbits.com/deepdi),[12](https://github.com/DeepBitsTechnology/DeepDi),[13](https://www.cs.ucr.edu/~heng/pubs/DeepDi.pdf)). Taking in consideration code section size-disassembly time correlations, the CPU version has a throughput of 146 KB/s and the GPU one is 170 times faster at around 24.5 MB/s([13](https://www.cs.ucr.edu/~heng/pubs/DeepDi.pdf)).

### Conclusion

At the moment, state-of-the-art disassemblers are extraordinary, powerful tools used in domains such as reverse engineering, software development and debugging(8), but still suffer from challenges like the ambiguity of instructions, the absence of symbols and debugging information and mostly from the presence of obfuscation techniques([6](https://www.retroreversing.com/disassemblers)), but these difficulties will begin to become a problem of the past with the evolution of machine learning.
